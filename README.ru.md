...
**Про этот worktree.** `warden do` не правит ветку, на которую вы смотрите. Он нарезает
свежий worktree от вашей фактической текущей ветки и работает там — через `git worktree` по
умолчанию, либо через [Orca](https://www.onorca.dev/download), если она запущена; тогда
worktree виден на её доске и в мобильном приложении. `--isolation git|orca` решает явно.
Гарантия у обоих одна, и если изоляцию сделать не удалось, команда отказывает, а не
сваливается на правку вашего чекаута.

Свежий `git worktree` пуст на всё, что нужно вашим проверкам, поэтому `setup:` в
`project.yaml` говорит, что в нём выполнить — `npm ci` или ничего для большинства проектов.
Когда worktree сделала Orca, она прогоняет собственный setup репозитория.

`--in-place` пропускает изоляцию и разрешает реализатору править дерево, в котором вы стоите —
нормально для одноразового репозитория и неверно для всего остального.
...
`--prepare off|auto|always` (по умолчанию `off`) отдельно от этой остановки. `off` оставляет
сегодняшний детерминированный `TaskDraft`. `auto` вызывает read-only планировщика только
если у task id ещё нет контракта на диске, и переиспользует готовый контракт только если
он сохраняет цель, scope и risk вызова. `always` вызывает его и тогда. Планировщик
черновит; Warden проверяет. `--prepare always --draft-only` платит планировщику и не
платит реализатору.
...
Сцена Unity, окно Tauri, canvas, которым харнесс не управляет: `visual_qa.evidence: agent`
на задаче пропускает браузерную стадию, и роль сама снимает картинки. Когда прогон
останавливается на человеке, эти картинки открываются на карточке Orca — отвечающий с
телефона смотрит на экран, о котором вердикт, а не на дайджест.

Кто занимает стадию, с каким effort и на каком раннере — накладка на один прогон, а не
правка `~/.warden`: `--use <стадия>=<профиль>`, `--effort <стадия>=<уровень>`,
`--host <стадия>=orca` в командной строке или та же карта как `use:` в задаче. Каждая
проверяется по ростеру до первого платного вызова, а заголовок прогона печатает `cast` —
какой профиль занимает какую стадию, — так что смену ростера видно через `--dry-run`, а не
по счёту. Гейт остановки предлагает ещё `advance`: он стартует следующий run задачи после
того, как вы закрыли блокер, и отказывается, если с момента остановки ничего не менялось.
   ...
   └ prompts/ schemas/                      └ runs/          улики (runs/.gitignore)
                    ...
                    └──────────────────┬─────────────────┘
                             ...
                             warden do "<цель>"
                                       │
                 ┌─────────────────────┴───────────────────────┐
                 │  Orca · черновик (или планировщик) · линтер │
...
То же правило держит весь блок `metrics` в `warden ledger`: у каждой измеряемой величины
рядом с суммой лежат `known_count` и `unknown_count`, а `total` равен `null`, когда не
сообщил никто. Что там считается и что сознательно не считается —
...
`warden ledger` агрегирует уже все прогоны проекта. `--global` читает домашний корпус
вне репозитория, фильтр — identity проекта; без фильтра — импортированные измерения
с неизвестной identity. `--import` копирует выбранные локальные runs и архивы.
...
Исчерпанная квота — единственный провал, который может исправить другой вендор. Warden
отделяет её от обычной ошибки и от временного rate limit и умеет передать роль другому
вендору, но **по умолчанию спрашивает**:
...
Резолвер не выпускает профиль, чья проба ни разу не запускалась: `verification.verified_on` —
это дата, когда собственная проба профиля была выполнена и прошла. Поэтому смена вендора за
ролью — правка конфигурации и одна команда, а не церемония.

```text
warden profiles                              что грузится, что пригодно и почему нет
warden profiles --verify codex-implement     прогнать пробу и проставить дату, если прошла
```

`--verify` запускает `verification.probe` профиля, сохраняет транскрипт в
`~/.warden/verification/`, печатает `what_to_check` и проставляет дату, когда проба прошла.

Транскрипт всё равно читайте. `what_to_check` — это место, где выясняется, какой ключ конверта
несёт ответ, спрашивает ли вендор подтверждения и сообщает ли он стоимость вообще; ничего из
этого нет в его документации. Чего дата **не** утверждает — что вы это прочли: раньше поле
заявляло человеческое суждение, и ничто этого не обеспечивало, то есть на гарантию полагались
все, а держал её никто.

Держит на самом деле другое и механически: read-only роль, которая написала, получает
`role_violated_read_only`, и её артефакт выбрасывается независимо от любых флагов и дат.
