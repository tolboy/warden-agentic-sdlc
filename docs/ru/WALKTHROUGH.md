...
Проба прошла — дата проставлена. `verified_on` записывает, что собственная проба этого профиля
была выполнена и прошла; это ровно то, что резолверу надо знать, прежде чем отправлять флаги,
которых никто никогда не запускал.

Раньше на бумаге поле означало больше — что человек сверил транскрипт с `what_to_check`. Ничто
этого не обеспечивало: штамп был командой, и тому, кто её набрал, верили на слово. Заявленный
смысл, которого инструмент не держит, хуже простого факта, поэтому теперь поле говорит факт.

`what_to_check` всё равно читайте. Всё ценное в этих профилях установлено именно так: какой
ключ конверта несёт ответ, открывает ли вендор интерактивную сессию, сообщает ли он стоимость.
Ничего из этого нет в документации вендора.
...
Необязательно, и выключено, пока не назовёте: `defaults.baseline_checks: fast` гоняет этот
набор **до первого вендора**. Красный набор — `baseline_failed`, никто не запускается.
Существующие проекты без ключа сохраняют поведение, которое у этого walkthrough было всегда.
...
Что не показано пошагово именно в этом разборе:

- **Живой цикл роли через `runner: orca`.** Теперь он доказан отдельно: Claude Opus 5 был
  виден в desktop Agent Dashboard, запустил объявленный гейт проекта, отправил типизированный
  `worker_done`, после чего Warden подтвердил delivery и освободил worker. См. датированную
  запись 2026-09-04 в [`../SMOKE.md`](../SMOKE.md). Сам адаптер worktree не создаёт;
  `OrcaIsolation` остаётся другим путём кода.
