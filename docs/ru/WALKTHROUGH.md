...
### Как петля проверяет кнопку

Это ответ на самый частый вопрос, и он состоит из трёх слоёв, а не из одного:

1. **Харнесс нажимает и утверждает.** Настоящий браузер по CDP находит контрол, кликает,
   проверяет, что после клика видно то, что обещано, и снимает кадр после каждого шага.
   Красный сценарий — это `visual_qa_failed` и раунд правок реализатору, ограниченный
   `max_fix_attempts`.
2. **Роль `visual_qa` смотрит на кадры.** Она отвечает только на то, чего машина измерить не
   может: обрезано, перекрыто, нарисовано не там. Её возражения — это ещё один раунд правок,
   а если она возражает и после них, прогон уходит к человеку с `visual_findings_remain`.
3. **Человек принимает.** Последнее слово всегда за ним, и ландинга петля не делает никогда.

Ревьюера в этом предложении нет, и это не упущение. Он читает диф и ничего не запускает.
Смешать зрение в ревью значило бы платить второму вендору за пиксели, которыми уже владеет
харнесс, и сломать `require_independent_vendor` ради работы, которая не является чтением дифа.
Если кнопки нет вовсе, ни один из слоёв не нужен сверх первого: сценарий красный, реализатор
получает упавший шаг и скриншот.
