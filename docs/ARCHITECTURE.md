...
| Browser tab "Warden · решение" | The pending decision with a button per option: the goal, each stage's result and who read it, the changed files, the screenshots and a note field. Opened when a run inside an Orca worktree stops for a person, and by `warden decide <run-id>` for one already waiting. A button records through the same path as `warden approve`, so a moved candidate or a changed decision is refused on the page; the Orca gate and `warden approve` answer it just as well. The Agent Dashboard lists agent sessions only, so the decision is a tab in the run's worktree rather than a dashboard row | `DecisionPage` on 127.0.0.1 under a random path token, opened with `orca tab create`; `Main.awaitDecision` waits for the page, the CLI or the gate |
